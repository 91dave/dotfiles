...
`WireframeBlock` content. Read it in full before authoring ANY wireframe; do not
author wireframes from memory.
...
existing html mockup, do not regenerate the frame. Edit that snippet in place in
the MDX source and rerun `plan local check` for the plan folder. A regenerated
frame quietly loses the details you were not thinking about at the time.
